...
function Dishes(props) {
  ...
  // Asegurar que dishes siempre sea un array
  const safeDishes = Array.isArray(dishes) ? dishes : [];
  const [searchResult, setSearchResult] = useState(safeDishes);
  const options = {
  ...
  const searchDish = async (e) => {
    ...
    const fuse = new Fuse(safeDishes, options);
    const result = fuse
      .search(term)
      ...
      <div className="heightFixAdmin px-6 lg:py-20 sm:py-16 py-12">
        <div className="mx-auto max-w-screen-xl">
          ...
          <div className="overflow-y-auto hideScrollBar h-96 p-1">
            {(searchTerm ? searchResult : safeDishes).map(
